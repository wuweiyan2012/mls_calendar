...
function drawCld(SY,SM) {
    ...
    for(i=0;i<42;i++) {
        ...
        lObj=eval('LD'+ (i+1));
        sObj.className = 'day';
        lObj.className = 'lunar';
        sD = i - cld.firstWeek;
        if(sD>-1 && sD<cld.length) { //日期内
            sObj.innerHTML = sD+1;
            sObj.setAttribute("daynum", sD+1);
            ...
            sObj.innerHTML = '&nbsp;';
            lObj.innerHTML = '&nbsp;';
            sObj.setAttribute("daynum", "");
        }
    }
}

function drawCldPanel()
{
    var gNum;
    var pHtml = "";
    for(i=0;i<6;i++) {
        pHtml += '<ul class="cldr-list clearfix" align=center>';
        for(j=0;j<7;j++) {
            var pItem = "";
            gNum = i*7+j;
            pItem += '<li id="GD' + (gNum +1) +'"><font id="SD' + (gNum +1) +'"';
            pItem += ' TITLE=""></font><br><font id="LD' + (gNum +1) + '"></font></li>';
            pHtml += pItem;
        }
        pHtml += "</ul>";
    }
    document.getElementById("cldr-panel").innerHTML = pHtml;
}
